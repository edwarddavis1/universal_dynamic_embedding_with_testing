...
class DynWalks(object):
    ...
    def sampling_traning(self):
        ...
        w2v = gensim.models.Word2Vec(sentences=None, size=self.emb_dim, window=self.window, sg=1, hs=0, negative=self.negative, ns_exponent=0.75,
                                     alpha=0.025, min_alpha=0.0001, min_count=5, sample=0.001, iter=10, workers=self.workers, seed=self.seed,
        ...
        for t in range(len(self.G_dynamic)):
            ...
            if t == 0:   # offline ==============================
                ...
                sentences = simulate_walks(
                    nx_graph=G0, num_walks=self.num_walks, walk_length=self.walk_length)
                ...
                node_update_list, self.reservoir = node_selecting_scheme(
                    graph_t0=G0, graph_t1=G1, reservoir_dict=self.reservoir, limit=self.limit, scheme=self.scheme)
                # print(node_update_list)
                # node_update_list_2_txt(node_update_list,'node_update_list.txt')
                sentences = simulate_walks(nx_graph=G1, num_walks=self.num_walks,
                                           walk_length=self.walk_length, affected_nodes=node_update_list)
