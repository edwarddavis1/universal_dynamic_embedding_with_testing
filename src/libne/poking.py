...
class DynWalks(object):
    def __init__(self, G_dynamic, limit, num_walks, walk_length, window, emb_dim, negative, workers, seed, scheme):
        ...
        # emb_dict @ t0, t1, ...; len(self.emb_dicts) == len(self.G_dynamic)
        self.emb_dicts = []
    ...
    def sampling_traning(self):
        ...
        w2v = gensim.models.Word2Vec(sentences=None, size=emb_dim, window=window, sg=1, hs=0, negative=negative, ns_exponent=0.75,
                                     alpha=0.025, min_alpha=0.0001, min_count=5, sample=0.001, iter=10, workers=workers, seed=seed,
                                     corpus_file=None, sorted_vocab=1, batch_words=128, compute_loss=False,
                                     max_vocab_size=None, max_final_vocab=None, trim_rule=None)  # w2v constructor, default parameters

        for t in range(len(G_dynamic)):
            t1 = time.time()
            if t == 0:   # offline ==============================
                G0 = G_dynamic[t]    # initial graph
                sentences = simulate_walks(
                    nx_graph=G0, num_walks=num_walks, walk_length=walk_length)
                sentences = [[str(j) for j in i] for i in sentences]
                # init traning, so update False
                w2v.build_vocab(sentences=sentences, update=False)
                # follow w2v constructor
                w2v.train(sentences=sentences,
                          total_examples=w2v.corpus_count, epochs=w2v.iter)

            else:   # incremental adapting ==============================
                G0 = G_dynamic[t-1]  # previous graph
                G1 = G_dynamic[t]    # current graph
                node_update_list, reservoir = node_selecting_scheme(
                    graph_t0=G0, graph_t1=G1, reservoir_dict=reservoir, limit=limit, scheme=scheme)
                # print(node_update_list)
                # node_update_list_2_txt(node_update_list,'node_update_list.txt')
                sentences = simulate_walks(nx_graph=G1, num_walks=num_walks,
                                           walk_length=walk_length, affected_nodes=node_update_list)
...
def update_reservoir_dict(G0, G1, reservoir_dict, exist_node_affected):
    for node in exist_node_affected:
        ...
        changes = len(nbrs_set1.union(nbrs_set0) -
                      nbrs_set1.intersection(nbrs_set0))  # steam edges
        if node in reservoir_dict.keys():
            # accumulated changes if already in reservoir
            reservoir_dict[node] += changes
        else:
            # newly added changes if not in reservoir
            reservoir_dict[node] = changes
...
def node_selecting_scheme(graph_t0, graph_t1, reservoir_dict, limit=0.1, scheme=4):
    ...
    G0 = graph_t0.copy()
    G1 = graph_t1.copy()
    # one may directly use steam added edges if possible
    ...
    if len(node_del) != 0:
        ...
        for node in node_del:
            if node in reservoir_key_list:
                # if a node is deleted, also delete it from reservoir
    ...
    #                 try:
    #                     node_scores.append(
    #                         math.exp(reservoir_dict[node]/G0.degree[node]))
    #                 except:
    ...
    #             remaining_pool_scores_prob.append(
    #                 remaining_pool_score[i]/remaining_pool_score_sum)
    #         diverse_nodes.extend(np.random.choice(
    #             remaining_pool, size=empty_part_counter, replace=True,  p=remaining_pool_scores_prob))
...
def simulate_walks(nx_graph, num_walks, walk_length, restart_prob=None, affected_nodes=None):
    ...
    # simulate walks on every node in the graph [offline]
    if affected_nodes == None:
        nodes = list(G.nodes())
    ...
    if restart_prob == None:  # naive random walk
        ...
        for walk_iter in range(num_walks):
            ...
            for node in nodes:
                walks.append(random_walk(
                    nx_graph=G, start_node=node, walk_length=walk_length))
        t2 = time.time()
        # print(f'random walk sampling, time cost: {(t2-t1):.2f}')
        ...
        for walk_iter in range(num_walks):
            random.shuffle(nodes)
            for node in nodes:
                walks.append(random_walk_restart(
                    nx_graph=G, start_node=node, walk_length=walk_length, restart_prob=restart_prob))
        t2 = time.time()
        # print(f'random walk sampling, time cost: {(t2-t1):.2f}')
    return walks
