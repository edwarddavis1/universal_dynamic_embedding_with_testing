...
d = 5
method = "URLSE"
YAs = regularised_ULSE(
    As, d, sparse_matrix=True, regulariser="auto", flat=False, verbose=True
)
# %%
...
def compute_diss_matrix(YAs, Z, cont_to_look_at, mat_type="dissimilarity"):
    ...
    for i in range(diss_matrix.shape[0]):
        for j in range(i + 1, diss_matrix.shape[1]):
            ...
            if mat_type == "dissimilarity":
                p_val = vector_displacement_test(ya_i_euro, ya_j_euro)
...
for cont_to_look_at in ["EU", "NA", "AS", "SA", "OC", "AF"]:
    diss_matrix = compute_diss_matrix(YAs, Z, cont_to_look_at, mat_type=mat_type)

    if mat_type == "dissimilarity":
        np.save(
            "saved_flight_matrices/{}_{}_matrix".format(method, cont_to_look_at),
            diss_matrix,
        )

        plot_diss_mat(
            diss_matrix,
            savename="saved_flight_matrices/{}_{}_matrix".format(
                method, cont_to_look_at
            ),
        )
    else:
        np.save(
            "saved_flight_matrices/{}_{}_p_value_matrix".format(
                method, cont_to_look_at
            ),
            diss_matrix,
        )
...
def plot_dendrogram(model, labels, **kwargs):
    ...
    dendrogram(
        linkage_matrix, **kwargs, labels=labels, leaf_rotation=90, leaf_font_size=18
    )
...
plot_dendrogram(model, truncate_mode="level", p=15, labels=labels, orientation="left")
...
# First split
plt.annotate(
    "Not Fully-Affected by COVID-19",
    xy=(0.5, 0.5),
    xytext=(0.3, 0.5),
...
conts_title = ["Europe", "Asia", "North America", "South America", "Africa", "Oceania"]
months = [
    "January",
    "February",
    "March",
...
mats = [
    compute_diss_matrix(YAs, Z, cont_to_look_at=continents[i])
...
plt.savefig(save_dir + "/all_dissimilarity_matrices_d={}_mutligraph.pdf".format(d))
